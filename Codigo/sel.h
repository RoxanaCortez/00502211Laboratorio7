...
Matrix createLocalK(int e,mesh &m){
    ...
    float t = m.getParameter(const_t);
    ...
    productRealMatrix((t*J)/(4*6*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,Beta,2,2,6),6,2,6),matrixA);
    ...
    //Preparando matrixH 
    float d = m.getParameter(const_d);
    productRealMatrix(-(d*Ae)/(8*D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixH);
...
Vector createLocalb(int e,mesh &m){
    ...
    float y_x = m.getParameter(const_y_X);
    float y_y = m.getParameter(const_y_Y);
    ...
    zeroes(b0,6);
    productMatrixVector(g_matrix,y,b0);
